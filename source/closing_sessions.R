  ...
  if (input$close_open_strata==T & ( length(input_strato_open())>0 | length(input_strato_close())>0 ) ) {
    ...
    withProgress(message = "Uploadig data to remote server:",{
      ...
      if(nrow(up)>0) {
        ...
        up=up[,list(id_battello,var,day=Sys.Date(),year=strftime(Sys.Date(),"%Y"),pr_i=round(pr_i,8),
        ...
        up=data.table(id_battello=0,var="control_var",day=Sys.Date(),year=strftime(Sys.Date(),"%Y"),pr_i=0,hist_value=0,hist_parameter=0,
