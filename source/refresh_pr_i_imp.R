...
flotta_temp=flotta[.( selected_strata_in_imp() )][,list(id_strato,lft,id_battello)]

if(nrow(flotta_temp)>0) { # selected_strata_in_imp() può essere nullo se l'utente non seleziona nulla. Non è vero per selected_strata() del tab outliers, dove se tutti i box sono nulli, equivalgono a selezionare tutto.
