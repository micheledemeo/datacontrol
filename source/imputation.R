...
data_for_imputation=reactive({
  ...
  if (input_group_for_imputation_method()=='strata') {
    ...
    imputation_output=all[in_imputation_temp, nomatch=0,][sent==1, list(id_strato,var,value)]
    ...
    imputation_output[value>=out_down & value<=out_up, list(var,id_strato,value)]
    ...
    imputation_output=all[in_imputation_temp, nomatch=0,][sent==1, list(codsis199,var,value)]
    ...
    imputation_output[value>=out_down & value<=out_up, list(var,codsis199,value)]
    ...
    imputation_output=all[in_imputation_temp, nomatch=0,][sent==1, list(codlft199,codsis199,var,value)]
    ...
    imputation_output[value>=out_down & value<=out_up, list(var,codlft199,codsis199,value)]
    
  }  else {
    setkey(all, var)
    in_imputation_temp=in_imputation[,.N,keyby=list(var)]
    imputation_output=all[in_imputation_temp, nomatch=0,][sent==1, list(var,value)]
    imputation_output[,c('out_up','out_down'):=list( quantile(value,.75)+1.5*IQR(value), quantile(value,.25)-1.5*IQR(value) ),  keyby=c('var')]
    imputation_output[value>=out_down & value<=out_up, list(var,value)]
