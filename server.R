...
observe({
  if (nrow(all[grepl(session_info, notes,fixed = T)])>0 & input$upload_button==T) {
    withProgress(message = "Uploadig data to remote server:",{
      ...
      up=up[,list(id_battello,var,day=Sys.Date(),year=year_local,pr_i=round(pr_i,8),hist_value=value_ok,hist_parameter=parameter_ok,hist_notes=notes,closing_session="open")]
...
output$version_nr=renderText({ paste('2.3.0:',year_local) }) #
